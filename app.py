...
st.caption("Autor: Claudio Navarrete Jara • Fuente: EVTD B / CSN / ChileAlerta / GAEL")
...
with st.sidebar:
    ...
    evtdb_pages = st.slider("Número de páginas EVTDB", 1, 5, 2)
...
try:
    ...
    st.error(f"⚠️ No fue posible cargar datos: {e}"); st.stop()

hasta = pd.Timestamp.now(tz="UTC"); desde = hasta - pd.Timedelta(days=int(dias))
...
if not mostrar_mapa:
    ...
    def color_for(r):
        if color_mode=="profundidad":
            v=r['profundidad']
            if v is None or np.isnan(v): return "#777777"
    ...
    for _,r in valid_map.iterrows():
        popup=f"<b>Mag:</b> {r.get('magnitud')} • <b>Prof:</b> {r.get('profundidad')} km<br><b>Fecha:</b> {r.get('fecha_local')}<br><b>Ref:</b> {r.get('referencia','')}"
...
st.markdown("---")
st.caption("Proyecto Solemne II • Ingeniería USS • Fuentes: EVTD B / CSN / ChileAlerta / GAEL • Hecho con Streamlit")
